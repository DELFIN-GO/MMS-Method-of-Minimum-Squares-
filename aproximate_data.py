...
def plot(path, plot_color):
    data = open(path)
    plot_data = open("datafile" + path, "w")
    ...
    plt.plot(Aproximate_data_x, Aproximate_data_y, alpha=0.25, color=plot_color)
    plt.plot(x_plot, y_plot, color=plot_color)
...
plot('spectrum_cold.dat', 'b')
plot('spectrum_hot.dat', 'r')
plt.ylim(-110, -40)
plt.grid(True)
plt.show()
